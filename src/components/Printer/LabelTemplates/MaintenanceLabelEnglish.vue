...
<template>
  <div ref="labelPreview" class="label-preview" :style="previewStyle">
    ...
    <div class="content-section" :style="contentStyle">
      <p class="text-row">Name: {{ props.customerName }}</p>
      <p class="text-row">Model: {{ props.equipmentModel }}</p>
      <p class="text-row">Fault: {{ props.fault }}</p>
      <p class="text-row">Phone Number: {{ props.phoneNumber }}</p>
    </div>
  </div>
</template>
